...
func (t errorTag) identifyer() string {
...
func parseTags(data [][]interface{}) []errorTag {
	uniqueTags := map[errorTag]interface{}{}

	for _, t := range data {
		for _, tagName := range t[6].([]interface{}) {
			uniqueTags[errorTag(tagName.(string))] = nil
		}
	}

	tags := make([]errorTag, 0, len(uniqueTags))
